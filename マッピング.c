...
int reader_kaiki_cb(FILE *fp,int masternumber)//ファイルの末尾にマスターファイルの中身のコードボーダーを付け足す回帰関数
{
	int kari,rl;
...
int reader_kaiki_ab(FILE *fp,int masternumber)//ファイルの末尾にマスターファイルの中身を付け足すテスト用回帰関数
{
	int kari,rl;
...
int reader_kaiki_rb(FILE *fp,int masternumber)//ファイルの末尾にマスターファイルの中身のリズムを付け足すテスト用回帰関数
{
	int kari,rl;
...
int borders_mapper(int masternumber,int unit,char *outputfilename)//ボーダーファイルを量子マッピングする関数
{
	...
	n = (m.length) / unit;
//x = (OUT*)calloc(n,sizeof(BOX));for(i=0;i<n;i++){x[i] = ini;} <= なにこれ?
