...
static const char *partname = "Eguitar";static int NOP;//起動時での他のパート数を記録しておく
...
static int Maker_Eguitar_Passage(int passageID,SCORE_LINE *s,X *x,int n)//横並びファイル作成すべき(音高のみ)
{
	//本日のセッティング
...
LoadSession(OP,NofOP,1,passageID);//for(i=0;i<64;i++){printf("%s [%2d] => ",partname,i);for(j=0;j<NofOP;j++){printf(",%3d",OP[i][j].note);}printf("\n");}

	for(i=0;i<NofOP;i++){free(OP[i]);OP[i]=NULL;}
	free(OP);OP=NULL;
///////////////////////////////////////////////////////
	for(i=0;i<n;i++){x[i].wave = i;}
	for(i=0;i<n;i++){x[i].pre = 40;}//この時点で音域に無理がないように
	for(i=0;i<n;i++)
	{//if(s[i].rh>0)
		if(1){nowchord.dia = s[i].chord_root;nowchord.kind= s[i].chord_kind;x[i].note = NearChordTone(x[i].pre,0,1,&nowchord,3);}else{x[i].note = -1;}
	}
	for(i=0;i<n;i++)
	{
		if(x[i].note>0 && s[i].ah>0){nowchord.dia = s[i].chord_root;nowchord.kind= s[i].chord_kind;x[i].note = NearChordTone(1+x[i].note,0,1,&nowchord,0);}
	}
///////////////////////////////////////////////////////
if(passageID== 2|| passageID==3 || passageID==4){for(i=0;i<n;i++){x[i].wave = 0;x[i].pre = 0;x[i].note = 0;}}
	ID = MakeNewScrapFile(s[i].passage_number);
	if(ID>0){scrapfp = OpenScrapFileW(ID);for(i=0;s[i].passage_number==ID;i++){fprintf(scrapfp,"%d,%d,%d\n",x[i].pre,x[i].wave,x[i].note);}fclose(scrapfp);}
	else{ScrapLoad(&x[i],s[i].passage_number,n);}
	
	for(i=0,outnote=0;i<n;i++){if(x[i].note>0){outnote=x[i].note;}fprintf(mysoundfp,"%d\n",outnote);}
	fclose(mysoundfp);
	AddSession(1,passageID,mysoundfn);
	return 0;
}

int Maker_Eguitar(char *scorefn)//TrackMaker(scorefn,partname)にすべき?
{int NofPL;
	char test[1000];int ID;//必ずしも楽節ひとまとまりとは限らないので ID としておく//int now_passage=(-1);
	X *x;USTFILEINFO finf = {120,1};
...
printf("PASSAGE[%d]は[%d]行で1セット by Maker_%s()\n",s[i].passage_number,NofPL,partname);
			Maker_Eguitar_Passage(s[i].passage_number,&s[i],&x[i],NofPL);
		}
	}
//*/
//////////////////////////////////メモリ上の操作はここまで
...
//////////////////////////////////////////////////以下出力
	...
	ust_file_out(partname,&finf);//このGUITAR関数では6弦分(つまり6回)呼び出さないといけない
	return 0;
}
