...
static const char *partname = "Ebase";static int NOP;//起動時での他のパート数を記録しておく
...
static int LoadSession(PART **part,int NofPart,int movementID,int passageID)
{
	...
	else{if(n!=NofPart){printf("引数[%d]と実際のパート数[%d]が合いません\n",NofPart,n);exit(1);}}
	for(i=0;i<m;i++)
	{
...
static int Maker_Ebase_Passage(int passageID,SCORE_LINE *s,X *x,int n)//横並びファイル作成すべき(音高のみ)
{
...
LoadSession(OP,NofOP,1,passageID);
	//for(i=0;i<64;i++){printf("[%d] => ",i);for(j=0;j<NofOP;j++){printf(",%3d",OP[i][j].note);}printf("\n");}
	...
	{//if(s[i].rh>0)
		if(1)//アタック
		{
			nowchord.dia = s[i].chord_root;nowchord.kind= s[i].chord_kind;x[i].note = NearChordTone(x[i].pre,0,1,&nowchord,1);}else{x[i].note = -1;}}ID = MakeNewScrapFile(s[i].passage_number);
			if(ID>0)
			{
...
int Maker_Ebase(char *scorefn)//TrackMaker(scorefn,partname)にすべき?
{int NofPL;
...
printf("PASSAGE[%d]は[%d]行で1セット by Maker_%s()\n",s[i].passage_number,NofPL,partname);
			Maker_Ebase_Passage(s[i].passage_number,&s[i],&x[i],NofPL);
		}
	}
//*/
//////////////////////////////////メモリ上の操作はここまで
